...
| Resource streams             | `backend/refresh/resourcestream`                                                                                                                                             | Stream rows must match snapshot row shape.                                                 |
...
| Object catalog               | `backend/objectcatalog`, `backend/refresh/snapshot/catalog.go`                                                                                                               | Source of truth for discovery, identity, browse, namespaces, and cluster listings.         |
| Shared resource semantics    | `backend/resourcemodel`                                                                                                                                                      | Owns identity, status, facts, lifecycle, and links.                                        |
| Rich detail/actions          | `backend/resources`, `backend/object_detail_provider.go`                                                                                                                     | Detail and imperative operations only, not list/table refresh data.                        |
...
| Node maintenance             | `backend/nodemaintenance`, `backend/refresh/snapshot/node_maintenance.go`                                                                                                    | Long-running maintenance/drain state is refresh-backed.                                    |
...
| Refresher names/config       | `frontend/src/core/refresh/refresherTypes.ts`, `frontend/src/core/refresh/refresherConfig.ts`                      | Keep manual refresh, intervals, cooldowns, and diagnostics aligned.                |
| Refresh execution            | `frontend/src/core/refresh/orchestrator.ts`, `frontend/src/core/refresh/RefreshManager.ts`                         | Orchestrator fetches; manager schedules.                                           |
| Refresh client               | `frontend/src/core/refresh/client.ts`                                                                              | Only approved direct `fetch` path.                                                 |
| Refresh diagnostics          | `frontend/src/core/refresh/components/diagnostics/diagnosticsPanelConfig.ts`                                       | Backend domains without frontend mappings create diagnostics gaps.                 |
...
| Settings                     | `frontend/src/ui/settings`, `frontend/src/core/settings`                                                           | Preferences, persistence, and settings UI.                                         |
...
| Logs/shell/port-forward/drain      | Focused backend session/stream tests plus UI lifecycle tests when present                     |
