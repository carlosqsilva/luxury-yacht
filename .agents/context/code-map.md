...
| Resource streams             | `backend/refresh/resourcestream`, `backend/refresh/resourcestream/stream_descriptor_dispatch.go`, `backend/refresh/resourcestream/stream_registration_*.go`                  | Most kinds stream via the registry's `Stream` descriptor facet (`registerDescriptorStreams`); `stream_registration_*` hold bespoke handlers. Rows must match snapshot row shape; WebSocket resource scopes are single-cluster. |
...
| Resource kinds (identity/model/facts) | `backend/resources/<kind>`, `backend/kind/kindregistry`, `backend/resourcekind`, `backend/resourcecontract`                                                          | Each built-in kind is one package + one `kindregistry.All` entry; subsystems loop the registry by facet. See `docs/architecture/resource-kind-registry.md`. |
| Shared resource-model primitives | `backend/resourcemodel`                                                                                                                                                  | Shared status/facts/link primitives + relationship index that per-kind models build on.    |
| Rich detail/actions          | `backend/resources/<kind>` (`details.go`, `actions.go`), `backend/object_detail_provider.go` (generated dispatch via `go generate ./backend`)                                | Detail and imperative operations only, not list/table refresh data. Typed detail fetchers require exact-GVK capability metadata. |
...
| Permission checks            | `backend/capabilities`, `backend/app_permissions.go`, `backend/resource_permission.go`, `backend/refresh/system/permission_gate.go`, `backend/refresh/system/registrations.go`, `backend/refresh/snapshot/permission.go`             | Keep QueryPermissions, action checks, refresh gates, diagnostics, and runtime checks aligned. |
